...
//
// DEBUG debug with # in URL
// mydomain.cn/#debug to start
// mydomain.cn/#stopdebug to stop
// debug stored in cookie
//
if (window.location.hash === '#debug') {
  createCookie('wxdebug', 'true', 1)
}
else if (window.location.hash === '#stopdebug') {
  alert ('debug removed')
  eraseCookie('wxdebug')
}

if (readCookie('wxdebug')) {
  var debugState = true
  alert ('wxdebug enabled, #stopdebug to stop debug')
}
...
// redirect if ? after end URL to avoid messing w wechat signature
if (window.location.search) window.location = window.location.origin + window.location.pathname

// only start the script if window.wxSharing detected
var configChecker = setInterval(function() {
  if(
    !window.wxSharing ||
    !window.wxSharing.appid ||
    !window.wxSharing.title ||
    !window.wxSharing.desc ||
    !window.wxSharing.link ||
    !window.wxSharing.imgUrl
    ) {
    console.warn('wxSharing: missing parameters')
  } else {
    if (!window.wxSharing.debug) window.wxSharing.debug = false

    clearInterval(configChecker)

    getSignature (window.location.origin, window.location.pathname, function(e){
      window.initSharingConfig = e.target.response.content
      initShareOnWechat(window.initSharingConfig)
    })
...
function watchForUrlChange () {
  ...
  var urlChecker = setInterval(function() {
    ...
    if (newURL !== oldURL) {
      ...
      al('changeurl from '+oldURL+' to '+newURL)
      if (iOS()) {
        // On iOS, we need to provide the initial SharingConfig if the URL is changed by JS (frontend frameworks)
        // https://github.com/vuejs/vue-router/issues/481
        console.log('URL CHANGED BUT IOS, reuse same signature')
        initShareOnWechat(window.initSharingConfig)
        al('ios')
      } else {
        al('android or else')
      ...
      // getSignature (window.location.origin, '/');
      oldURL = newURL;
...
function getSignature (originUrl, pathnameUrl, callback) {
  var url = window.wxSharing.apiUrl+'/api/index.php?action=getsignature&appid='+window.wxSharing.appid+'&appurl='+originUrl+'&signuri='+pathnameUrl

  var oReq = new XMLHttpRequest();
  oReq.onload = function(e) {
    if (debugState) console.log(e)
    callback(e)
  };
...
function initShareOnWechat (wxparams) {
  wx.config({
    debug: debugState,
  ...
  wx.ready(function(){
    var shconfig = {
      title: window.wxSharing.title,
      desc: window.wxSharing.desc,
      link: window.wxSharing.link,
      imgUrl: window.wxSharing.imgUrl,
      success: function () {
        if (window.wxSharing.success) window.wxSharing.success()

      },
      cancel: function () {
        if (window.wxSharing.cancel) window.wxSharing.cancel()
      }
    }
